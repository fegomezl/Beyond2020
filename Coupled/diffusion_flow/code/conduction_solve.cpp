...
void Conduction_Operator::Mult(const Vector &X, Vector &dX_dt) const{
    ...
    rhs_theta.AddBoundaryIntegrator(new BoundaryLFIntegrator(r_robin_h_ref_theta), robin_bdr_theta);
    rhs_theta.Assemble();
    rhs_theta.ParallelAssemble(Rhs_theta);
    Z_theta += Rhs_theta;

    rhs_phi.AddBoundaryIntegrator(new BoundaryLFIntegrator(r_robin_h_ref_phi), robin_bdr_phi);
    rhs_phi.Assemble();
...
int Conduction_Operator::SUNImplicitSetup(const Vector &X, const Vector &B, int j_update, int *j_status, double scaled_dt){
    ...
    dt_r_robin_h_theta.SetAConst(scaled_dt);     dt_r_robin_h_phi.SetAConst(scaled_dt);
    dt_r_robin_h_ref_theta.SetAConst(scaled_dt); dt_r_robin_h_ref_phi.SetAConst(scaled_dt);
    ...
    t_theta->AddBoundaryIntegrator(new MassIntegrator(dt_r_robin_h_theta), robin_bdr_theta);
    t_theta->Assemble();
    ...
    t_phi->AddBoundaryIntegrator(new MassIntegrator(dt_r_robin_h_phi), robin_bdr_phi);
    t_phi->Assemble();
...
int Conduction_Operator::SUNImplicitSolve(const Vector &B, Vector &X, double tol){
    ...
    rhs_theta.AddBoundaryIntegrator(new BoundaryLFIntegrator(dt_r_robin_h_ref_theta), robin_bdr_theta);
    rhs_theta.Assemble();
    rhs_theta.ParallelAssemble(Rhs_theta);
    Z_theta += Rhs_theta;

    rhs_phi.AddBoundaryIntegrator(new BoundaryLFIntegrator(dt_r_robin_h_ref_phi), robin_bdr_phi);
    rhs_phi.Assemble();
    rhs_phi.ParallelAssemble(Rhs_phi);
    Z_phi += Rhs_phi;
