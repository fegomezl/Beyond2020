...
//Boundary conditions
double boundary_vorticity_f(const Vector &x, double t);

double boundary_stream_f(const Vector &x, double t);
double boundary_stream_in_f(const Vector &x, double t);
double boundary_stream_out_f(const Vector &x, double t);
...
double boundary_stream_f(const Vector &x, double t){
    ...
    return tanh(5*t/RelaxationTime)*InflowFlux*in*out;
}

//Boundary condition for stream at inlet
double boundary_stream_in_f(const Vector &x, double t){
    double x_rel = x(0)/RIn;
    return tanh(5*t/RelaxationTime)*InflowFlux*pow(x_rel, 2)*(2-pow(x_rel, 2));
}

//Boundary condition for stream at outlet
double boundary_stream_out_f(const Vector &x, double t){
    double y_rel = x(1)/ZOut;
    return tanh(5*t/RelaxationTime)*InflowFlux*pow(y_rel, 2)*(3-2*y_rel);
}
