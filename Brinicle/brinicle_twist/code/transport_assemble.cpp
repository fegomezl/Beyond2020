    ...
    T0_solver(MPI_COMM_WORLD), T1_solver(MPI_COMM_WORLD)
{
    ...
    if (!config.restart){
        ...
        ConstantCoefficient coeff_boundary_temperature(InflowSalinity);
        ...
        ConstantCoefficient coeff_boundary_salinity(InflowTemperature);
...
double initial_temperature_f(const Vector &x){
    ...
    if (NucleationRegion)
        return NucleationSalinity;//NucleationTemperature;
    else
        return InitialSalinity;//InitialTemperature;
}
...
double initial_salinity_f(const Vector &x){
    ...
    if (NucleationRegion)
        return NucleationTemperature;//NucleationSalinity;
    else
        return InitialTemperature;//InitialSalinity;
}
