...
void Artic_sea::time_step(){
    ...
    if (last || vis_steps <= vis_iteration){
        ...
        for (int ii = 0; ii < phase->Size(); ii++)
            (*phase)(ii) = Phase((*salinity)(ii), (*temperature)(ii));
...
void Transport_Operator::SetParameters(const BlockVector &X, const Vector &rVelocity){
    ...
    for (int ii = 0; ii < phase.Size(); ii++){
        heat_inertia(ii) = HeatInertia(salinity(ii), temperature(ii));
        heat_diffusivity(ii) = HeatDiffusivity(salinity(ii), temperature(ii));
        salt_diffusivity(ii) = SaltDiffusivity(salinity(ii), temperature(ii));;

        phase(ii) = Phase(salinity(ii), temperature(ii));
        temperature(ii) = salinity(ii) - FusionPoint(temperature(ii));
    }
    ...
    m1.AddDomainIntegrator(new MassIntegrator(coeff_rM));  
    m1.Assemble();                                        
    m1.Finalize();                                        
    ...
    k0.AddDomainIntegrator(new DiffusionIntegrator(coeff_rD1));
    ...
    k1.AddDomainIntegrator(new DiffusionIntegrator(coeff_rD0));
    k1.AddDomainIntegrator(new ConvectionIntegrator(coeff_rMV));
    k1.Assemble();
    k1.Finalize();
...
void Flow_Operator::SetParameters(const BlockVector &X){
    ...
    for (int ii = 0; ii < impermeability.Size(); ii++){
        ...
        impermeability(ii) = Impermeability(S, T);
        density(ii) = Density(S, T);
    }
