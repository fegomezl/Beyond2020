...
Transport_Operator::Transport_Operator(Config config, ParFiniteElementSpace &fespace_H1, ParFiniteElementSpace &fespace_ND, int dim, int attributes, Array<int> block_offsets_H1, BlockVector &X):
    ...
    phase(&fespace_H1), 
    heat_diffusivity(&fespace_H1), salt_diffusivity(&fespace_H1), fusion_diffusivity(&fespace_H1),
    salinity_gradient(&fespace_ND), rvelocity(&fespace_ND),
    coeff_r(r_f), 
    coeff_zero(dim, zero_f),
    coeff_rD0(coeff_r, coeff_r),
    coeff_rD1(coeff_r, coeff_r),
    coeff_rD2(coeff_r, coeff_r),
    coeff_rV(&rvelocity),
    ...
    gradient(&fespace_H1, &fespace_ND)
{
    ...
    if (!config.restart){
        ...
        ConstantCoefficient coeff_boundary_temperature(TStoHS(InflowTemperature, InflowSalinity));
