...
double Impermeability(const double H); 
...
double Phase(const double H){
    //Dimentionless
    return 0.25*(H+1)*tanh(5*EpsilonInv*(H+1))
         - 0.25*(H-1)*tanh(5*EpsilonInv*(H-1))
         + 0.5;
}

double HeatDiffusivity(const double H){ 
    return constants.d_temperature_s*(0.5*(1 - tanh(5*EpsilonInv*(H+1))))
         + constants.d_temperature_l*(0.5*(1 + tanh(5*EpsilonInv*(H-1))));
}

double SaltDiffusivity(const double H){
    return constants.d_salinity_s + (constants.d_salinity_l-constants.d_salinity_s)*Phase(H);
}

double FusionDiffusivity(const double H, const double S){
    return (constants.FusionPoint_a + 3*constants.FusionPoint_b*pow(S, 2))*(constants.kL_temperature_s + (constants.kL_temperature_l-constants.kL_temperature_s)*Phase(H));
}
...
double ExpansivityTemperature(const double H, const double S){
    double T_ = HStoTS(H, S);//T_bounded(HStoTS(H,S));
    double S_ = S;//S_bounded(S);
    return Phase(H)*constants.Buoyancy_k*(
...
double TStoHS(const double T, const double S){
    ...
    return (constants.TStoHS_s*T_rel-1)*0.5*(1-tanh(5*EpsilonInv*T_rel))
         + (constants.TStoHS_l*T_rel+1)*0.5*(1+tanh(5*EpsilonInv*T_rel));
}
 
double HStoTS(const double H, const double S){
    double T_rel = (constants.HStoTS_s*(H+1))*0.5*(1-tanh(5*EpsilonInv*(H+1))) 
                 + (constants.HStoTS_l*(H-1))*0.5*(1+tanh(5*EpsilonInv*(H-1)));
