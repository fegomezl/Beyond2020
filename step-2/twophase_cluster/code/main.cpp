...
int main(int argc, char *argv[]){
    ...
    args.AddOption(&config.n_DeltaT, "-DeltaT", "--DeltaT",
    ...
    int i_min = config.refinements;
    int j_min = config.n_DeltaT;

    int N_i = 2;
    int N_j = 2, m = 4;

    for (config.refinements = i_min; config.refinements < i_min + N_i; config.refinements++){
        for (config.n_DeltaT = j_min; config.n_DeltaT < j_min + m*N_j; config.n_DeltaT += m){
            //Run the program for different refinements
            InvDeltaT = pow(10, config.n_DeltaT);
