...
void Flow_Operator::Update_T(const HypreParVector *Theta){
    ...
    VectorGridFunctionCoefficient grad_theta(theta_grad_aux);
    InnerProductCoefficient r_hat_grad_theta(r_hat, grad_theta);
    ProductCoefficient F(-19.77, r_hat_grad_theta);
    ProductCoefficient rF(r, F);
    ProductCoefficient neg_rF(neg, rF);*/
    
    theta_aux->GetDerivative(1, 0, *theta_grad_aux);
    GridFunctionCoefficient grad_theta(theta_grad_aux);
    ProductCoefficient F(-19.77, grad_theta);
    ProductCoefficient rF(r, F);
    ProductCoefficient neg_rF(neg, rF);
