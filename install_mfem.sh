...
make SLEPC_DIR=${INSTALL_DIR}/petsc PETSC_DIR=/home/wind/Library/MFEM/boyan/petsc PETSC_ARCH="" check
...
cmake -DCMAKE_INSTALL_PREFIX=${SUNDIALS_DIR}/install -DEXAMPLES_INSTALL_PATH=${SUNDIALS_DIR}/install/examples -DENABLE_MPI:BOOL=ON -DENABLE_LAPACK:BOOL=ON -DENABLE_PETSC:BOOL=ON -DENABLE_HYPRE:BOOL=ON -DHYPRE_INCLUDE_DIR=${PETSC_DIR}/include -DHYPRE_LIBRARY_DIR=${PETSC_DIR}/lib DLAPACK_LIBRARIES=${PETSC_DIR}/lib -DPETSC_ARCH= -DPETSC_DIR=${PETSC_DIR} -DSUNDIALS_INDEX_SIZE=32 ${SUNDIALS_DIR}
