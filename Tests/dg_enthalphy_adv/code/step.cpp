...
void Transport_Operator::SetParameters(const Vector &X){
    ...
    //Create RHS
    ConstantCoefficient bdr_s(H_s), bdr_l(H_l), coeff(10.);
    if (b) delete b;
    if (B) delete B;
    b = new ParLinearForm(&fespace);
    b->AddBdrFaceIntegrator(new DGDirichletLFIntegrator(bdr_l, coeff_rA, config.sigma, config.kappa), ess_bdr);
    b->AddBdrFaceIntegrator(new BoundaryFlowIntegrator(bdr_l, coeff_rV, -1.), ess_bdr);
    b->Assemble();
